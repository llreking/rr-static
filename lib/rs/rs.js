!(function ($) {
    ...
    function Cascade(option) {
        ...
        this.settings = $.extend({ title: '请选择',valuename:'id',textname:'text', data: [], labels: ['省份','城市','区县'],tip:'请先选择上一级' ,onselect:null,cancel:null,textJoin:' ',zIndex:80000000,maxWidth:767,dark:false}, option);
        this.target = option.el;
        var $b = $('<div class="bbox-mask cascade"><div class="bbox-container" style="max-width:' + (typeof (this.settings.maxWidth) == 'number'||(!(this.settings.maxWidth+'').contains('%')&&!(this.settings.maxWidth+'').contains('px')) ? this.settings.maxWidth + 'px' : this.settings.maxWidth) + '"><div class="bbox'+(this.settings.dark?' dark':'')+'"><div class="bbox-header"></div><div class="bbox-body"></div><div class="bbox-footer"><a class="bbox-btn csc-cancel">取消</a><a class="bbox-btn csc-confirm">确定</a></div></div></div></div>');
    ...
    $.bbox = function (option) {
        function BBox(op) {
            ...
            this.settings = { title: "", content: "hello",success:null ,maxWidth:767,dark:false};
            this.idx = bboxIdx;
            $.extend(this.settings, option);
            this.create = function () {
                var $b = $('<div class="bbox-mask"><div class="bbox-container" style="max-width:' + (typeof (this.settings.maxWidth) == 'number'||(!(this.settings.maxWidth+'').contains('%')&&!(this.settings.maxWidth+'').contains('px')) ? this.settings.maxWidth + 'px' : this.settings.maxWidth) + '"><div class="bbox'+(this.settings.dark?' dark':'')+'"><div class="bbox-header"></div><div class="bbox-body"></div><div class="bbox-footer"></div></div></div></div>');
    ...
    $.alert = $.confirm = function (option) {
        function ConfirmBox(op) {
            var that = this;
            this.settings = { title: "提示", content: "hello", buttons: { "确定": { action: null } }, success: null, maxWidth: 350, titleCenter: true,contentCenter:true,showClose:false,dark:false};
            this.id = 'confirmboxbox-' + getGuid();
            $.extend(this.settings, option);
            this.create = function () {
                var $c = $('<div class="confirmbox-mask"><div class="confirmbox-container" style="max-width:' + (typeof (this.settings.maxWidth) == 'number' || (!(this.settings.maxWidth + '').contains('%') && !(this.settings.maxWidth + '').contains('px')) ? this.settings.maxWidth + 'px' : this.settings.maxWidth) + '"><div class="confirmbox'+(this.settings.dark?' dark':'')+'"><div class="confirmbox-header"' + (this.settings.titleCenter ? ' style="justify-content:center"' : '') + '></div><div class="confirmbox-body"' + (this.settings.contentCenter ? 'style=" text-align:center"' : '') + '></div><div class="confirmbox-footer"></div></div></div></div>');
                $c.prop('id', this.id);
...
function checkDevTool() {
    var threshold=160;
    var opened = window.outerWidth - window.innerWidth > threshold || window.outerHeight - window.innerHeight > threshold;
    if (opened) {
